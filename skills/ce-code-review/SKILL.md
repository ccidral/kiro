...
| **`mode:agent`** | One JSON object (see ### JSON output format below) + the same the tmp skill/.../ce-code-review/<run-id>/` artifacts |
...
Emit **one raw JSON object** as the primary response — a single bare JSON value, **no markdown code fence**. A leading ```` ```json ```` fence makes the response start with backticks and breaks naive `JSON.parse` consumers, so never wrap it. Also write `review.json` under the tmp skill/compound-engineering/ce-code-review/<run-id>/` with the same payload.
...
- Include the run-artifact path when one was written: the tmp skill/compound-engineering/ce-code-review/<run-id>/`
...
Always write run artifacts under the tmp skill/compound-engineering/ce-code-review/<run-id>/`:
