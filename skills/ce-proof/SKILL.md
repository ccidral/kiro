...
The JSON response includes the markdown, API links, and agent auth hints. Use the state skill when you need mutation metadata, marks, or presence:
...
Comment, suggestion, and rewrite operations go to `POST https://www.proofeditor.ai/api/agent/{slug}/ops`. Block edits use the api skill/agent/{slug}/edit/v2`.
...
1. **Literal repeated change:** use the edit skill/v2` with `find_replace_in_doc` (optionally constrained by `fromRef`, `toRef`, or `block_filter`). This is the fastest and least error-prone path for terminology renames, punctuation/style sweeps, and other exact text substitutions.
2. **Known block or section change:** use the edit skill/v2` block operations from a fresh the snapshot skill: `replace_block`, `insert_before`, `insert_after`, `delete_block`, `replace_range`, or `find_replace_in_block`.
3. **Visible track-changes desired:** use the ops skill `suggestion.add` (pending or `status: "accepted"`) when the user should see a suggestion mark and reject/revert affordance for that specific edit.
...
When in doubt, start with the snapshot skill and build a small the edit skill/v2` batch. A narrow failed edit is easier to inspect and retry than a broad rewrite, and it avoids clobbering concurrent human work.

**Retry discipline after mutation errors — verify before retrying.** An error response is not proof that nothing was written.

- `STALE_BASE`, `BASE_TOKEN_REQUIRED`, `MISSING_BASE`, `INVALID_BASE_TOKEN` — pre-commit, token-related. Re-read the state skill, rebuild the request body with a fresh `baseToken`, and retry once with a new `Idempotency-Key`.
...
Batch the ops skill supports `comment.reply`, `comment.resolve`, and `comment.unresolve` for existing threads. Use it for HITL ingest passes instead of issuing separate reply and resolve requests per thread.
...
Prefer `find_replace_in_doc` or block-level the edit skill/v2` operations first. `rewrite.apply` is broad, disruptive, and blocked while live clients are connected.
...
Read the snapshot skill to get block `ref` IDs and `mutationBase.token`. `ref` values are opaque request tokens tied to the snapshot/baseToken; re-read the snapshot skill before follow-up block edits if writes have landed. `operations` commits atomically — either every op lands or none do — so one the edit skill/v2` call can batch dozens of block edits safely and efficiently (see the bulk-sweep guidance in `references/hitl-review.md` Phase 2.4). Successful full responses include the next `mutationBase.token` and fresh `snapshot.blocks[].ref` values for chaining.
...
| POST | the rewrite skill | Last-resort whole-doc replacement: `{"content":"full markdown","by":"ai:compound-engineering"}` |
...
2. Read the document via content negotiation on the shared URL or via the api skill/agent/{slug}/state` when you need marks/mutation metadata
3. For content edits, prefer the edit skill/v2` `find_replace_in_doc` or block operations; use the ops skill for comments, suggestions, and comment replies/resolution
...
# For content edits, prefer the edit skill/v2 over rewrite.apply.
...
- Prefer `find_replace_in_doc` and block-level the edit skill/v2` edits before considering `rewrite.apply`
